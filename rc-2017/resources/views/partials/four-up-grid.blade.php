<!--<div class="four-up-grid__container">
  @for ($i = 0; $i < 4; $i++)
    <div class="four-up-grid__service">
      <div class="four-up-grid__image">
        <img src="http://via.placeholder.com/200x200">
      </div>
      <div class="four-up-grid__header">
        General Butt
      </div>
      <p class="four-up-grid__description">
        DAVID Construction is a team of highly-skilled professionals comprised of engineers, architects and talented crafts people. We have earned a solid reputation for supplying creative solutions to difficult construction projects as one of the most successful general contractors in Louisville, Kentucky.
      </p>
    </div>
   @endfor
</div> -->

<div class="four-up-grid__container">
    <div class="four-up-grid__service">
      <div class="four-up-grid__image">
        <img src="@asset('images/icons/new-home-icon.png')" width="200" height="200" alt="New Home Construction">
      </div>
      <div class="four-up-grid__header">
        New Home Construction
      </div>
      <p class="four-up-grid__description">
        Since 1995, Colyer Construction has been the gold standard of new home construction. With clarity and expertise, our team talks through ever aspect and builds your dream home, exactly how you imagined it.
      </p>
    </div>

    <div class="four-up-grid__service">
      <div class="four-up-grid__image">
        <img src="@asset('images/icons/remodeling-icon.png')" width="200" height="200" alt="Remodeling">
      </div>
      <div class="four-up-grid__header">
        Remodeling
      </div>
      <p class="four-up-grid__description">
        Interested in adding to your existing dream home? Our team has done over 100 remodels in the last decade alone. Our work speaks for itself. Take a look through our gallery to see some examples of beautiful addons.
      </p>
    </div>

    <div class="four-up-grid__service">
      <div class="four-up-grid__image">
        <div class="four-up-grid__design-build-icon">
          <img src="@asset('images/icons/design-build-icon.png')" width="190" height="190" alt="Design Build Services">
        </div>
      </div>
      <div class="four-up-grid__header">
        Design Build Services
      </div>
      <p class="four-up-grid__description">
        Having a hard time imagining your perfect home? Colyer Construction utilizes cutting edge technology to help you see visually how your home will turn out--before the first brick is layed.
      </p>
    </div>

    <div class="four-up-grid__service">
      <div class="four-up-grid__image">
        <div class="four-up-grid__who-we-are-icon">
          <img src="@asset('images/icons/about-us-icon.png')" width="190" height="190" alt="Who We Are">
        </div>
      </div>
      <div class="four-up-grid__header">
        Who We Are
      </div>
      <p class="four-up-grid__description">
        Ron Colyer has been succesfully building homes for the past decades. With expertise, he will focus on creating the most quality end result--no shortcuts.
      </p>
    </div>
</div>
